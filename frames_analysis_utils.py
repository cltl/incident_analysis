...
def plot_figure(plot_data, output_folder, condition, incident, verbose, specified_bbox_to_anchor, specified_figsize, dimension, language):
    """plot a figure"""
    if specified_figsize != None:
        plt.figure(figsize=specified_figsize, dpi=300)
    else:
    ...
    if specified_bbox_to_anchor != None:
        legend = plt.legend(bbox_to_anchor=specified_bbox_to_anchor)
    else:
        legend = plt.legend()

    if language != None:
        pdf_path = f"{output_folder}/output/{incident}/figures/{dimension}/{dimension}_{condition}_{language}.pdf"
    else:
        pdf_path = f"{output_folder}/output/{incident}/figures/{dimension}/{dimension}_{condition}.pdf"

    plt.savefig(pdf_path, bbox_extra_artist=legend, bbox_inches='tight')
    if verbose:
        print(f"exported plot to {pdf_path}")
    return
...
def type_selection(condition, dimension, incident_d, output_folder, incident, ordered_l, language, verbose):
    ...
    if condition == "anchor":
        for time_bucket, info in incident_d[condition].items(): #iterate over time_buckets:info
        ...
        if dimension == "frames":
            ...
            anchor_lus_distribution_to_latex(latex_l, output_folder, incident, condition, language, verbose)
    else:
        climax_l = []
        for time_bucket, info in incident_d[condition].items(): #iterate over time_buckets:info
            if dimension in info.keys(): #check if frames are present
                ...
                for tupl in most_common:
                    ...
                    types_s.add(token)
                    tb_l.append((token, freq, perc))
                    if verbose >= 2:
                        print(token, freq, perc)
                        print()
                climax_l.append((time_bucket, tb_l))
            else:
                n_fes = 0
                type_selection = set()
                counter = {}
        ...
        ordered_latex_l = []
        for tb in ordered_l:
            for tupl in climax_l:
                if tb == tupl[0]:
                    ordered_latex_l.append(tupl)
        #climax_distribution_to_latex(ordered_latex_l, output_folder, incident, condition, verbose)
    return counter_d, types_s, n_tokens_d

def restructure_data(incident_d, condition, ordered_l, dimension, output_folder, incident, language, verbose):
    """restructure particpant data for plotting"""
    plot_data = {} #{frame: {x: [day1, day2, day3], y: [percentage]}}

    counter_d, types_s, n_tokens_d = type_selection(condition, dimension, incident_d, output_folder, incident, ordered_l, language, verbose)
